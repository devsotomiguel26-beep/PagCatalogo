-- ============================================================================
-- FIX TIMESTAMP TIMEZONE - ENFOQUE CORRECTO
-- ============================================================================
-- SOLUCIÓN: Eliminar TODAS las vistas primero, hacer ALTER, recrear vistas
-- ============================================================================

-- ============================================================================
-- PASO 1: Eliminar TODAS las vistas conocidas con CASCADE
-- ============================================================================
-- CASCADE asegura que si hay vistas dependientes de estas vistas,
-- también se eliminen
...
-- Por si acaso, eliminar cualquier otra vista posible del sistema
DROP VIEW IF EXISTS archived_requests CASCADE;
DROP VIEW IF EXISTS test_requests CASCADE;
DROP VIEW IF EXISTS cancelled_requests CASCADE;
DROP VIEW IF EXISTS expired_requests CASCADE;
DROP VIEW IF EXISTS delivered_requests CASCADE;
DROP VIEW IF EXISTS paid_requests CASCADE;

-- ============================================================================
-- PASO 2: Ahora SÍ podemos convertir las columnas timestamp a timestamptz
-- ============================================================================

-- Convertir created_at (la principal)
...
-- Convertir download_links_expires_at
ALTER TABLE photo_requests
...
-- ============================================================================
-- PASO 3: Recrear las vistas esenciales del sistema
-- ============================================================================
...
COMMENT ON VIEW pending_earnings IS 'Ganancias pendientes - soporta ambas estructuras JSON';
...
COMMENT ON VIEW photographer_earnings_summary IS 'Resumen de ganancias por fotógrafo';
...
COMMENT ON VIEW director_earnings_summary IS 'Resumen de ganancias totales del director';
...
COMMENT ON VIEW settlements_detail IS 'Detalle completo de todas las liquidaciones';
...
COMMENT ON VIEW adjustments_history IS 'Historial completo de ajustes manuales';
...
--
-- 2. Verificar Hugo Cerda (debe mostrar hora correcta):
-- SELECT
--   client_name,
--   created_at,
...
--
-- ✅ Resultado esperado:
-- - data_type: 'timestamp with time zone' para todas las columnas
